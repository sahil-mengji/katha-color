...
export function getMoodFilters(moodId) {
  ...
  const preset =
    customMoods[moodId] || MOOD_PRESETS[moodId] || MOOD_PRESETS.default;
...
// Get custom moods from localStorage
function getCustomMoods() {
  if (typeof window === "undefined") return {};
  try {
    const saved = localStorage.getItem("katha-custom-moods");
    return saved ? JSON.parse(saved) : {};
  } catch (error) {
    console.error("Failed to load custom moods:", error);
    return {};
  }
}
